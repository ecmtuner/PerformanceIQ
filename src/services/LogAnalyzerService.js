...
// ─── Main ─────────────────────────────────────────────────────────────────────
export async function analyzeLog({ fileContent, filename, uid, carName, gear, fuelType, engineFamily }) {
  ...
  if (uid) {
    try {
      await addDoc(collection(getDB(), 'logAnalyses'), {
        uid, filename, carName: carName || 'Unknown',
        healthScore: report.healthScore,
        fuelType, engineFamily, gear: gear || null,
        channelStatuses: Object.fromEntries(
          Object.entries(report.channels).map(([k, v]) => [k, v.status])
        ),
        timestamp: serverTimestamp(),
      });
    } catch (e) { console.warn('LogAnalyzer: Firestore log failed', e); }
  }
